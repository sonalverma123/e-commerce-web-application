...
export default function Bags() {
  ...
  const [bagsCategory, setBagsCategory] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const productsPerPage = 6;
  useEffect(() => {

    const fetchBagsProducts = async () => {
      try {
        const token = localStorage.getItem('token');

        const response = await axios.get('http://localhost:5000/api/auth/bags', {
          headers: { Authorization: token },
        });
        console.log('Bags Products:', response.data);

        setBagsCategory(response.data);
      } catch (error) {
